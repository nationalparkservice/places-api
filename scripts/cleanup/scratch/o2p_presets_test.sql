...
*/
    SELECT
      CASE
        WHEN "geometry" && ARRAY['point'] THEN "preset"
        ELSE null
      END as "preset",
      "pathname",
    ...
    FROM (
      SELECT
        ...
        "layerIndex",
        "geometry",
        each((select tags from nps_render_point where osm_id = 4968297)) AS "input_tags",
        "hstore_len"
      FROM (
        SELECT
          "preset",
        ...
        FROM (
          ...
          FROM
            (
              ...
              WHERE
                "inCarto" = true AND
                ((ARRAY['point'] && ARRAY['point'] AND "nps_presets"."geometry" && ARRAY['point']) OR
                (ARRAY['line','area'] && ARRAY['point'] AND "nps_presets"."geometry" && ARRAY['line','area']))
            ) "hstore_tag_list"
        ) "available_tags"
      ) "explode_tags"
    ) "paired_tags"
